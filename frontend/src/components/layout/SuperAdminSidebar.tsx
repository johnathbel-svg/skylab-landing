...
import {
    Crown, Database, Briefcase, TrendingUp, Users, BookOpen, Activity, Settings, LogOut, ChevronRight, ChevronLeft
} from "lucide-react"
...
const routes = [
    ...
    {
        label: "Proveedores API",
        icon: Briefcase,
        href: "/super-admin/providers",
        color: "#10B981",
    },
    {
        label: "Finanzas Sheets",
        icon: TrendingUp,
...
export function SuperAdminSidebar({ userEmail }: SuperAdminSidebarProps) {
    ...
    useEffect(() => {
        const stored = localStorage.getItem("super-admin-sidebar-collapsed")
        if (stored === "true") {
            setIsCollapsed(true)
        }
    }, [])
    ...
    const handleToggleCollapse = (collapsed: boolean) => {
        ...
        localStorage.setItem("super-admin-sidebar-collapsed", collapsed ? "true" : "false")
    }
    ...
    return (
        ...
        >
            ...
            >
                {!isCollapsed ? (
                    <div className="flex items-center gap-2">
                        {/* Imagen Oficial Horizontal con badge */}
                        <motion.div
                            animate={{ 
                                scale: [1, 1.03, 1],
                                filter: [
                            ...
                            className="relative flex-shrink-0 flex flex-col gap-0.5"
                        >
                            <Image 
                    ...
                    <div className="flex justify-center w-full">
                        {/* Imagen Oficial de Símbolo Solo */}
                        <motion.div
                            animate={{ 
                                scale: [1, 1.05, 1],
                                filter: [
                ...
                {!isCollapsed && (
                    <button
                        ...
                        className="p-1.5 rounded-lg hover:bg-white/5 text-slate-500 hover:text-white transition-colors animate-in fade-in"
            ...
            <div className="flex-1 py-5 relative z-10 overflow-y-auto">
                {!isCollapsed && (
                    <div className="px-5 text-[10px] font-black tracking-[0.18em] uppercase mb-3 text-slate-500">
                        Consola Admin
                    </div>
                )}
                ...
                <div className="space-y-1 px-3">
                    {routes.map((route) => {
                        const isActive = pathname === route.href || (route.href !== "/super-admin" && pathname.startsWith(route.href))
                        ...
                        return (
                            ...
                            >
                                ...
                                {isActive && !isCollapsed && (
                                    <ChevronRight className="w-3.5 h-3.5 opacity-40 animate-in slide-in-from-left-1" style={{ color: "#6366F1" }} />
                                )}
                            </Link>
                        )
                    })}
                    ...
                    {isCollapsed ? (
                        <div className="flex flex-col items-center gap-2">
                            ...
                            <div className="w-11 h-11 rounded-full flex items-center justify-center text-base font-black flex-shrink-0"
                                style={{
                                    background: "linear-gradient(135deg, rgba(99, 102, 241, 0.2), rgba(255, 255, 255, 0.08))",
                        ...
                        <>
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <div className="flex flex-col min-w-0">
                                    <span className="text-[13px] font-bold truncate text-slate-100">
                                        {userEmail}
                                    </span>
                            ...
                            <form action={signout} className="w-full">
                                <Button
                                    type="submit"
                                    variant="ghost"
                                    className="w-full h-9 justify-center font-bold text-[12px] rounded-lg transition-all hover:bg-rose-500/10 hover:text-rose-400"
                                    style={{
                ...
                <div className="flex items-center justify-center gap-2.5 pt-2 border-t border-white/[0.04] mt-2">
                    ...
                    {!isCollapsed && (
                        <span className="text-[10px] font-medium tracking-wide text-slate-500">
                            Skylab by{" "}
                            <span style={{ color: "#6366F1" }} className="font-bold">Synerg-IA</span>
                        </span>
                    )}
                </div>
            </div>
